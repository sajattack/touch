...
static int siw_touch_blocking_notifier_callback(struct notifier_block *this,
				   unsigned long event, void *data)
{
	struct siw_ts *ts =
...
int siw_touch_init_notify(struct siw_ts *ts)
{
	...
	if (ret < 0) {
		t_dev_err(dev, "failed to regiseter touch atomic_notify callback\n");
		goto out;
	}
